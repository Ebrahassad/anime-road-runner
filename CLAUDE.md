...
A 3D endless runner written directly against [`flutter_scene`](https://pub.dev/packages/flutter_scene) (Flutter GPU / Impeller) — no game engine.
...
fvm flutter build web --release --base-href /anime-road-runner/  # gh-pages build
```
...
- `assets/` now holds only runtime-loaded content, declared under `flutter: assets:` — `assets/models/dash.glb` (loaded via `Node.fromGlbAsset`), `assets/road/` and `assets/city_props/` (GLB scenery, same loader), `assets/sfx/*.wav` (played via `audioplayers`), and `assets/icon/` (app icon source + adaptive-icon foreground for `flutter_launcher_icons`). Unused preview media that previously lived at the asset root (screenshots/GIFs for an external README) has been removed — it was never bundled into the app and had no consumer in this repo. When adding runtime assets, put them in their own folder and declare it.
