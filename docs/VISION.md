...
This is an **original** project by the project owner. It is not a
clone of anything — the reference game below is a *look-and-feel target only*.
Everything in this repo is hand-written. Commits are authored by the project owner, in a
...
4. **Performance is a feature.** The project owner explicitly wants richer visuals *without*
...
6. **The honest ceiling of primitives.** Everything here is built from
   ...
   *polished low-poly*. To go beyond that (truly "authored" trees/rocks/props),
   ...
   it with the project owner before taking it; don't silently swap primitives for models.
...
- **Removed** — the old dust trail behind Dash, and the ambient floating
  "motes" (both were rejected in review).
...
Next, and needing the project owner's review first:
...
Feature depth & platforms (separate tracks the project owner picked):
...
- **You cannot headless-test the 3D.** `flutter_scene` needs a real GPU/Impeller
  ...
  Visual judgement is human — show the project owner a screenshot/clip and iterate.
...
- **Commits are the project owner's.** Don't commit as an agent; don't add co-author
