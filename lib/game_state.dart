// Part of the Anime Road Runner library — see lib/main.dart.
...
class _GamePageState extends State<GamePage>
  ...
  static const double coinRadius = 0.36; // upright disc, not a flat card
  ...
  static const int cCoin = 0xFFFFD54A; // coin gold, also its pickup sparkle
  ...
  static const int cSkyTop = 0xFF4B9BE8; // sky blue (screen top)
  static const int cSkyBot = 0xFFDCEEFF; // pale horizon
  static const int cGrass = 0xFF43883D; // grass field
  static const int cAsphaltA = 0xFF30343B; // road tile A
  static const int cAsphaltB = 0xFF3A4048; // road tile B
  static const int cHedge = 0xFF3F9B4F; // roadside greenery (repurposed posts)
  ...
  static const int cShoulder = 0xFF68645D; // packed dirt
  ...
  static const double rampImpulse = 11.5; // vs jumpImpulse 12
  ...
  static const int cBarrierA = 0xFFF04B3A; // barrier stripe (red)
  static const int cBarrierB = 0xFFE7E4DC; // barrier stripe (cream)
  static const int cBarrierLeg = 0xFF5A5F66; // barrier legs (grey)
  static const int cContainer = 0xFF356B50; // shipping container (green)
  ...
  // Tracks the short transition from Jump Start into the looping airborne pose.
  ...
  // Set if world/model asset loading (GLBs, textures) threw after the first
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      // World/model loading runs after the first frame is already on screen
      // (see main.dart), so a failure here must not freeze the app -- it
      // should degrade to "menu is visible but empty" rather than hang.
      try {
        await _buildWorld();
        if (!mounted) return;
  ...
  Future<void> _buildWorld() async {
    ...
    // Kenney straight city road — one continuous road piece repeated forward.
  ...
  Future<void> _loadDash() async {
    try {
      ...
      if (!mounted) return;
      // Animation mapping for dash.glb:
      // Sprint     -> normal running
      // Walk       -> menu / non-playing state
      // Jump Start -> first part of the jump
      // Jump       -> airborne loop
      // Jump Land  -> landing one-shot
      final runClip =
  ...
  void _updateDashAnim(double dt) {
    ...
    // While airborne, advance from Jump Start into Jump.
    if (!_grounded) {
      _jumpStartAnimT += dt;
    ...
    // Grounded: briefly show Jump Land, then return to Sprint.
    _clipIdle?.weight = 0.0;
    _clipJumpStart?.weight = 0.0;
    _clipJump?.weight = 0.0;
  ...
  /// City GLB roadside decoration.
  /// Visual replacement only; road, physics and gameplay remain unchanged.
  ...
  /// little bow on top.
  // --- anime obstacle shapes ---
  // Visual-only replacement. Spawn, collision, pooling and lane logic stay unchanged.

  /// Bright road crate with rounded-looking corner accents.
  Node _giftBox({required int boxHex}) {
    ...
    // Small glowing-looking top marker.
    root.add(
      Node(
        mesh: Mesh(
  ...
  /// Anime road barricade with a bold warning bar.
  Node _barrier() {
    final Node root = Node();
  ...
  /// Compact futuristic road container.
  ...
  /// Futuristic launch ramp with bright side rails.
  Node _rampNode() {
    final Node root = Node();
  ...
  Future<void> _loadLanguage() async {
    try {
      ...
      // If nothing is saved yet, AppStrings already defaulted from the
      // device locale (see l10n.dart) -- nothing to do.
    } catch (_) {}
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              // itself.
              if (_enteringName &&
                  _placeFor(_score.round()) != null) ...<Widget>[
                const SizedBox(height: 10),
                Text(
                  AppStrings.t('got_place').replaceFirst(
                      '%s', _ordinal(_placeFor(_score.round())!)),
                  style: const TextStyle(
                    color: cGold,
                    fontSize: 16,
                    fontWeight: FontWeight.w700,
  ...
  List<Widget> _nameEntry() {
    return <Widget>[
      Text(
        AppStrings.t('enter_name'),
        style: const TextStyle(color: cGold, fontSize: 15),
      ),
      const SizedBox(height: 10),
      SizedBox(
        width: 260,
      ...
      const SizedBox(height: 12),
      _primaryButton(AppStrings.t('save'), _submitName),
    ];
  }
  ...
  List<Widget> _crashButtons() {
    return <Widget>[
      Wrap(
        ...
        runSpacing: 12,
        children: <Widget>[
          _primaryButton(AppStrings.t('play_again'), _startGame),
          OutlinedButton(
            onPressed: _goMenu,
            style: OutlinedButton.styleFrom(
      ...
      OutlinedButton.icon(
        ...
        label: Text(AppStrings.t('share_score')),
        style: OutlinedButton.styleFrom(
  ...
  Future<void> _shareScore() async {
    final int s = _score.round();
    final String text = 'I scored $s in ${AppStrings.t('app_name')} — a 3D '
        'endless runner built in Flutter (Flutter GPU / Impeller).\n'
        '#Flutter #GameDev';
