...
class _Runner3DSceneState extends State<Runner3DScene> {
  ...
  Future<void> setupScene() async {
    ...
    threeJS.camera.lookAt(three.Vector3(0, 1.2, 5));

    final ambient = three.AmbientLight(0xffffff, 1.2);
    threeJS.scene.add(ambient);

    final sun = three.DirectionalLight(0xffffff, 1.8);
    sun.position.setValues(5, 15, -10);
    threeJS.scene.add(sun);

    final fill = three.PointLight(0x4fd1c5, 2.0, 40);
    fill.position.setValues(0, 5, -5);
  ...
  Future<void> _loadRoad() async {
    ...
    try {
      ...
      final roadGltf = await gltfLoader.fromAsset('assets/road-straight.glb');
      ...
      debugPrint('Road loading failed: $e');
    } finally {
      _roadLoading = false;
    }
  }
